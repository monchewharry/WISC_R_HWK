...
```
reject null hypothesis which means the mean is not zero
...
```
accept null hypothesis which means the series has no serial correlation 
## c
...
```
F-statistics is 30.8, the null hypothesis of no arch effect is rejected    
...
spec5 <- ugarchspec( 
  variance.model=list(model="sGARCH", garchOrder=c(1,1)),
  mean.model=list(armaOrder=c(3,2), include.mean=T, archm=T, archpow=2) , 
